...
- 搜索默认参数
  - 从运行时 JSON 的 search_defaults 字段读取，包含每源最大结果数、超时秒数、缓存过期间隔、默认数据源列表以及新增的语义过滤配置。
- HTTP 代理配置
...
- 配置项
  ...
  - default_sources：默认启用的数据源列表（**已更新**：从 ["duckduckgo", "arxiv"] 更改为 ["scholar", "arxiv"]）
  - enable_semantic_filter：AI 语义过滤全局开关（**新增**）
  - relevance_threshold：默认相关性阈值（0-100）
- 使用方式
  - 搜索服务在未显式传入 sources/filters 时，使用这些默认值。
  - 缓存键基于查询、数据源与过滤器生成，命中缓存可显著降低延迟与外部请求压力。
  - 语义过滤功能可通过 enable_semantic_filter 控制，relevance_threshold 设定过滤阈值。

**更新** 默认搜索源已从 DuckDuckGo 更改为 Google Scholar，这反映了对学术搜索场景的优化。

**章节来源**
- [backend/services/search_service.py](file://backend/services/search_service.py#L28-L67)
- [.qoder/config.json](file://.qoder/config.json#L8-L13)
- [README.md](file://README.md#L400-L418)
- [frontend/src/hooks/useSearch.js](file://frontend/src/hooks/useSearch.js#L4-L21)
...
- [backend/config.py](file://backend/config.py#L15-L85)
- [run.sh](file://run.sh#L39-L49)
- [README.md](file://README.md#L460-L467)
...
- 语义过滤性能
  - AI 语义过滤会增加 LLM 调用开销，可根据性能需求调整 enable_semantic_filter 和 relevance_threshold。
...
- 语义过滤问题
  - 若语义过滤导致性能问题，可在 .qoder/config.json 中设置 "enable_semantic_filter": false 来禁用该功能。
- 日志定位
...
- [README.md](file://README.md#L483-L497)

## 结论
本项目的配置体系以"环境变量 + 运行时 JSON + 默认值"的分层设计实现灵活可控的配置管理。通过明确的优先级与合并机制，结合速率限制、缓存与代理等关键参数，既满足开发调试的便捷性，也兼顾生产环境的安全与稳定性。最新的配置变更体现了对学术搜索场景的优化（默认使用 Google Scholar），并通过新增的语义过滤配置提供了更智能的结果筛选能力。建议在生产环境中配合配置中心与自动化部署流程，持续优化配置并加强安全审计。
...
- 从旧版配置迁移到 .qoder/config.json
  ...
  - **重要更新**：默认数据源已从 ["duckduckgo", "arxiv"] 更改为 ["scholar", "arxiv"]，请在迁移时注意这一变更。
- 新增配置项
  - 添加语义过滤相关配置：enable_semantic_filter（默认 true）和 relevance_threshold（默认 40）。
- 迁移步骤
...
#### 搜索默认配置
- max_results_per_source：每源最大结果数（默认 15）
- timeout_seconds：请求超时秒数（默认 60）
- cache_expire_hours：搜索结果缓存过期小时数（默认 24）
- default_sources：默认启用的数据源列表（**已更新**：["scholar", "arxiv"]）
- enable_semantic_filter：AI 语义过滤全局开关（**新增**：默认 true）
- relevance_threshold：默认相关性阈值（0-100，**新增**：默认 40）

#### 速率限制配置
- arxiv：容量 5，补充速率 0.33
- duckduckgo：容量 20，补充速率 2.0
- zhihu：容量 3，补充速率 0.2
- scholar：容量 10，补充速率 1.0

#### 分析服务配置
- provider：LLM 提供商（默认 deepseek）
- zhipu_model：智谱模型名称（默认 glm-4-flash）
- deepseek_model：DeepSeek 模型名称（默认 deepseek-chat）
- max_content_length：最大内容长度（默认 4000）
- temperature：温度参数（默认 0.7）
- cache_expire_days：分析结果缓存过期天数（默认 7）

#### 下载服务配置
- save_directory：保存目录（默认 data/downloads）
- max_concurrent_downloads：最大并发下载数（默认 3）
- arxiv_mirrors：arXiv 镜像地址列表（默认 https://arxiv.org/pdf/ 和 https://cn.arxiv.org/pdf/）

**章节来源**
- [.qoder/config.json](file://.qoder/config.json#L1-L31)
- [README.md](file://README.md#L400-L447)